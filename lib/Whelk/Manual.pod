...
an afterthought in Whelk, but rather welded into it's very core. Each time you
...
resource during app's construction and it's job is to define all the
...
Lastly, Whelk uses I<wrappers> to define how are requests and responses treated
...
generated resource. Tools like C<Swagger Editor|https://editor.swagger.io/> can
...
C<Whelk::Formatter>. It has to start with C<+> to avoid prefixing. By default,
value C<JSON> is used, which constructs L<Whelk::Formatter::JSON>. Can be
configured to C<YAML> available in Whelk's core, or any custom formatter.

=head4 api_wrapper

A default class for endpoint formatters. It will be prefixed with
C<Whelk::Wrapper>. It has to start with C<+> to avoid prefixing. By default,
value C<Simple> is used, which constructs L<Whelk::Wrapper::Simple>. Can be
configured to C<WithStatus> available in Whelk's core, or any custom wrapper.
...
Same as L<api_wrapper>, but specialized for this resource only.

=item formatter

Same as L<api_formatter>, but specialized for this resource only.
...
A hash containing OpenAPI data, with the following keys:
...
Same as L<api_formatter>, but specialized for OpenAPI endpont only.
...
Whelk-specific bits and pieces. It can cointain these keys:
